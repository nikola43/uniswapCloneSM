...
async function main() {
    ...
    let contractName = "SFYXBuyAndBurn"
    const contractFactory = await ethers.getContractFactory(contractName);
    SFYXBuyAndBurn = await contractFactory.deploy(factoryAddress, SFYAddress, SFYXAddress, WETH);

    console.log(colors.cyan("SFYXBuyAndBurn Address: ") + colors.yellow(SFYXBuyAndBurn.address));

    await test_util.sleep("360");
    //await test_util.updateABI(contractName)
    await test_util.verify(SFYXBuyAndBurn.address, contractName, [factoryAddress, SFYAddress, SFYXAddress, WETH])
